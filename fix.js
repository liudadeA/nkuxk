// ==UserScript==
// @name         南开大学选课助手(自动读取ID最终版)
// @namespace    http://tampermonkey.net/
// @version      2026-03-05
// @description  南开专属，自动读取profileId/semesterId，严格200OK防重复
// @author       You
...
(function () {
    ...
    function getProfileId() {
        ...
        log('❌ 无法自动读取 profileId，请检查是否在选课页面！');
        alert('⚠️ 选课助手警告：无法读取选课批次ID，请确保在正确的选课页面！');
        return null;
    }

    /**
     * 2. 自动读取 semesterId (学期ID，可选但推荐)
     */
    function getSemesterId() {
        ...
        // 方法3: 从 lessonJSONs 里反推 (如果有数据的话)
        if (window.lessonJSONs && window.lessonJSONs.length > 0) {
            // 尝试从刷新人数的接口缓存里找，或者直接用一个默认值不报错即可
            // 这里如果找不到，就留空，fetch时不带这个参数通常也能跑
        }

        // 兜底默认值 (如果之前的脚本用的是4384，先保留，能自动读到更好)
        return '4384';
    }
        ...
        panel.innerHTML = `
            ...
            <div class="mini-elect-body">
                ...
                <div class="id-display">
                    批次ID: ${CURRENT_PROFILE_ID || '读取失败'} | 学期ID: ${CURRENT_SEMESTER_ID}
                </div>
                ...
                <div class="mini-btns">
                    ...
                    <button class="mini-btn" id="miniStart" ${!CURRENT_PROFILE_ID ? 'disabled class="disabled"' : ''}>立即开始</button>
    ...
    function bindMainPanelEvents() {
        panel.querySelector('#miniClose').onclick = () => { clearTiming(); config.isRunning = false; panel.remove(); };
        ...
        function updateBtnStatus() {
            startBtn.disabled = config.isRunning || !CURRENT_PROFILE_ID;
            pauseBtn.disabled = !config.isRunning;
            stopBtn.disabled = !config.isRunning;
            pauseBtn.textContent = config.isPaused ? '恢复' : '暂停';
            if (config.timingEnabled) { startBtn.classList.add('disabled'); startBtn.disabled = true; }
            else if (CURRENT_PROFILE_ID) { startBtn.classList.remove('disabled'); }
        }

        startBtn.onclick = async () => {
            if (config.isRunning || !CURRENT_PROFILE_ID) return;
            if (config.PREFERRED_NOS.length === 0) { alert('请先填写课程号并点击【保存配置】！'); return; }
        ...
        pauseBtn.onclick = () => { config.isPaused = !config.isPaused; updateBtnStatus(); log(config.isPaused ? '已暂停' : '已恢复'); };
        stopBtn.onclick = () => { config.isRunning = false; config.isPaused = false; requestedIdsCache.clear(); updateBtnStatus(); log('已手动停止'); };
    }

    function bindDrag(targetPanel, headSelector) {
        const head = targetPanel.querySelector(headSelector);
        head.onmousedown = e => { isDragging = true; const rect = targetPanel.getBoundingClientRect(); offsetX = e.clientX - rect.left; offsetY = e.clientY - rect.top; targetPanel.style.transition = 'none'; };
        ...
        document.onmouseup = () => { isDragging = false; targetPanel.style.transition = 'all 0.2s'; };
    }
    ...
    function setTiming() {
        clearTiming();
        if (!CURRENT_PROFILE_ID) { alert('无法读取选课批次ID，无法使用定时功能！'); return; }
        if (config.PREFERRED_NOS.length === 0) { alert('请先填写课程号并保存配置！'); return; }
        ...
        config.timingTimer = setTimeout(() => {
            log('⏰ 定时时间到！');
            config.timingEnabled = false; updateTimingStatus();
            requestedIdsCache.clear();
            config.isRunning = true; config.isPaused = false;
        ...
        config.timingUpdateInterval = setInterval(() => {
            if (!config.timingEnabled) { clearInterval(config.timingUpdateInterval); return; }
    ...
    function clearTiming() {
        ...
        updateTimingStatus();
        const startBtn = document.getElementById('miniStart');
        if(startBtn && CURRENT_PROFILE_ID) { startBtn.disabled = false; startBtn.classList.remove('disabled'); }
    }
    ...
    function updateTimingStatus(diff = 0) {
        ...
        if (config.timingEnabled && diff > 0) {
            ...
            el.innerHTML = `<span class="timing-active">✅ 定时中: ${config.timingHour}:${config.timingMinute}:${config.timingSecond} (剩 ${h}:${m}:${s})</span>`;
        } else {
    ...
    async function refreshCount() {
        ...
        try {
            refreshElectedCache();
            // 使用动态 semesterId
            await fetch(`/eams/stdElectCourse!queryStdCount.action?projectId=1&semesterId=${CURRENT_SEMESTER_ID}&_=${Date.now()}`, { credentials: 'include' });
        } catch (e) {
    ...
    async function main() {
        ...
        const stopBtn = document.getElementById('miniStop');
        const updateBtns = () => {
             if(startBtn) startBtn.disabled = config.isRunning || !CURRENT_PROFILE_ID;
